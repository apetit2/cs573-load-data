...
export interface LinePlotProps<T extends CSVRow> {
  filter?: string;
...
export const LinePlot = <T extends CSVRow>({
  filter,
  grouping,
  width,
  height = 400,
  ...
  opacity = '.3',
  strokeWidth = 3,
}: PropsWithChildren<LinePlotProps<T>>) => {
        ...
        if (lines.has(row[grouping] as string)) {
          lines.set(row[grouping] as string, [
            ...lines.get(row[grouping] as string)!,
            {
              ...
              stroke: colorScale(colorValue(row)),
              strokeWidth,
              opacity,
            },
          ]);
